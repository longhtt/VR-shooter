...
int			hmdW = 1440, hmdH = 1600;			// Vive Cosmos, per eye: 1440 wide, 1700 high
...
// meshes
Mesh		bench, ground, head, leftHand, rightHand, FrontWall, leftWall, rightWall;
int			meshTextureUnit = 5;

// buttons
Toggler		annotate("Annotate", true, 20, 13, 14);
Toggler		stereopsis("Stereopsis", true, 140, 13, 14);
...
vec3 EyeOffset(Side e) {
	...
	float x = 0.0f;
	float z = 0.0f;
	return e == Left? vec3(x*headX+z*headZ) : vec3(-x*headX+z*headZ);
}
...
void RenderScene(Camera &camera, bool showHead) {
	...
	//SetUniform(s, "useLight", false);
	FrontWall.Display(camera, meshTextureUnit);
	leftWall.Display(camera, meshTextureUnit);
	rightWall.Display(camera, meshTextureUnit);
	//SetUniform(s, "useLight", true);
	if (showHead) {
		SetUniform(s, "useLight", false);
		head.Display(camera);
...
void MouseMove(float x, float y, bool leftDown, bool rightDown) {
	...
	if (leftDown) {
		...
		if (picked == &framer) {
			...
			if (!fixGaze.on) lookAt = Origin(head.toWorld)+f*ZAxis(head.toWorld);
			OrientHead();
		}
	}
}
...
void MakeScene() {
	...
	if (!ground.Read(objDir+"Floor.obj", imgDir+"marble_floor.jpg")) printf("can't read floor\n");
	if (!head.Read(objDir+"Head.obj")) printf("can't read head\n");
	if (!leftHand.Read(objDir+"Pistol_obj.obj", imgDir+"Pistol_color.jpg") || !rightHand.Read(objDir + "Hand.obj")) printf("can't read hand\n");
	if (!FrontWall.Read(objDir + "billBoard.obj", imgDir + "bilboard.png")) printf("can't read bench\n");
	//if (!FrontWall.Read(objDir + "billBoard.obj", imgDir + "marble_floor.jpg")) printf("can't read wall\n");
	if (!rightWall.Read(objDir + "billBoard.obj", imgDir + "Floor.tga")) printf("can't read wall\n");
	if (!leftWall.Read(objDir + "billBoard.obj", imgDir + "Floor.tga")) printf("can't read wall\n");
	bench.toWorld = Scale(.5f)*Translate(0, -.2f, .7f);
	ground.toWorld = Scale(5)*Translate(0, -.3f, 0);
	leftHand.toWorld = Translate(.7f, .4f, -.4f)*RotateX(-90)* RotateZ(90) *Scale(-.15f, .15f, .15f);
	rightHand.toWorld = Translate(.4f, .4f, -.4f)*RotateX(-90)*Scale(.15f);
	head.toWorld = RotateX(20)*Translate(.5f, .6f, -.85f)*Scale(.17f);
	FrontWall.toWorld = Scale(3) * Translate(0, 0.5, 2) * RotateX(90);
	leftWall.toWorld = Scale(3) * Translate(2, 0.5, 2) * RotateX(90);
	rightWall.toWorld = Scale(3) * Translate(-2, 0.5, 2) * RotateX(90);
	OrientHead();
}
