...
// for gameplay (shooting system)
bool		rightHandPicked = false;
bool		onTarget = false;
vec3		target;
bool		hit = false;
...
bool intersect(Mesh &m, vec3 p1, vec3 p2, float &alpha) {
	mat4 xM = Invert(m.toWorld);
	vec3 xp1 = Vec3(xM * vec4(p1, 1)), xp2 = Vec3(xM * vec4(p2, 1));
	return m.IntersectWithSegment(xp1, xp2, &alpha);
}
...
void TestFramer(float x, float y, Mesh &m, void *oldPicked) {
	...
	if (!picked && MouseOver(x, y, Origin(m.toWorld), cameraScene.fullview)) {
		picked = &framer;
		rightHandPicked = &m == &rightHand;
		cout << "TestFramer rightHandPicked = " << rightHandPicked << endl;
...
void MouseMove(float x, float y, bool leftDown, bool rightDown) {
	...
	if (leftDown) {
		...
		cout << "rightHandPicked from mouseMove = " << rightHandPicked << endl;
		if (rightHandPicked) {
			float alpha = 0;
			vec3 p1 = Origin(rightHand.toWorld), rFinger = FingerTip(Right), p2 = p1 + 5 * (rFinger - p1);
			onTarget = intersect(button, p1, p2, alpha);
			// cout << "onTarget = " << onTarget << endl;
			if (onTarget)
				target = p1 + alpha * (p2 - p1);
		}
