...
// buttons
Toggler		annotate("Annotate", false, 20, 13, 14);
...
void RenderHeadM(mat4 m, vec3 color) {
	GLuint s = UseMeshShader();
	mat4 save = head.toWorld;
	head.toWorld = m*Scale(.17f);
	SetUniform(s, "color", color);
	SetUniform(s, "twoSidedShading", true);
	head.Display(cameraScene);
	head.toWorld = save;
	UseDrawShader();
	float a = .25f;
	ArrowV(vec3(0,0,0), vec3(a,0,0), cameraScene.modelview*m, cameraScene.persp, red);
	ArrowV(vec3(0,0,0), vec3(0,a,0), cameraScene.modelview*m, cameraScene.persp, grn);
	ArrowV(vec3(0,0,0), vec3(0,0,a), cameraScene.modelview*m, cameraScene.persp, blu);
}
...
/*	SetUniform(s, "useLight", false);
	...
	if (showHead) {
		// original
		RenderHeadM(head.toWorld, grn);
		// back/front reflection
		vec4 x1(1,0,0,0), y1(0,1,0,0), z1(0,0,-1,0), o1(0,0,0,1);
// NO		RenderHeadM(Translate(-.5f, 0, 0)*mat4(x1, y1, z1, o1)*head.toWorld, yel);
		// left/right reflection
		vec4 x2(-1,0,0,0), y2(0,1,0,0), z2(0,0,1,0), o2(0,0,0,1);
		RenderHeadM(mat4(x2, y2, z2, o2)*head.toWorld, blu);
		// up/down reflection
		vec4 x3(1,0,0,0), y3(0,-1,0,0), z3(0,0,1,0), o3(0,0,0,1);
// NO		RenderHeadM(Translate(-1, 0, 0)*mat4(x3, y3, z3, o3)*head.toWorld, mag);
		// separation
		mat4 m = head.toWorld;
		mat3 r = mat3(Vec3(m.row[0]), Vec3(m.row[1]), Vec3(m.row[2]));
		vec3 t(m[0][3], m[1][3], m[2][3]);
		mat4 test = ReferenceFrame(r.row[0], r.row[1], r.row[2], t);
		RenderHeadM(Translate(.5f, 0, 0)*test, red);
	}
}
...
void Display() {
	...
	if (annotate.on) {
		...
		mat4 xButton = Invert(button.toWorld);
		vec3 xp1 = Vec3(xButton*vec4(p1, 1)), xp2 = Vec3(xButton*vec4(p2, 1));
		float alpha = 0;
		Line(p1, p2, 10, red);
		if (button.IntersectWithSegment(xp1, xp2, &alpha))
	...
	if (picked == &cameraScene && (float) (clock()-mouseEvent)/CLOCKS_PER_SEC < .5f)
...
void MakeScene() {
	...
	if (!leftHand.Read(objDir+"HandLeft.obj") ||
		!rightHand.Read(objDir+"HandRight.obj"))
			printf("can't read hand\n");
	leftHand.toWorld = Translate(.7f, .4f, -.4f)*RotateX(-90);
