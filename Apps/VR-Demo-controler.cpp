...
// 3) changes to main::... !vroom.StartTransformHMD(&head.toWorld)) {
//    really, there's no need to provide *mat4 here, and call in event loop:
//				vroom.UpdateTransformHMD();
//    should simply return the matrix
...
void Display() {
	...
	RenderEye(Right, red);			// red background
	if (vroom.HmdPresent())
...
//		if (!vroom.InitFrameBuffer(targetWidth, targetHeight))
		...
		if (hmdPresent) { // && !vroom.StartTransformHMD(&head.toWorld)) {
		//	hmdTrack.Set(false);
